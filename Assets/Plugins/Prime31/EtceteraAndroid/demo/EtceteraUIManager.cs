...
namespace Prime31
{
	public class EtceteraUIManager : MonoBehaviourGUI
	{
		...
		void OnGUI()
		{
			...
			if( GUILayout.Button( "Shared Resized Image" ) )
			{
				StartCoroutine( saveScreenshotToSDCard( path =>
				{
					if( EtceteraAndroid.resizeImageAtPath( path, Screen.width / 10, Screen.height / 10 ) )
						EtceteraAndroid.shareWithNativeShareIntent( "Check this out!", "Tiny Image", "Sharing a tiny screenshot and text...", path );
				} ) );
			}
